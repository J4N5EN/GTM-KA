# Parts of the game generation code is from:
# https://www.lutanho.net/play/hex.html
# Speicically the parts that make the "bots" play against each other, allowing us to simulate thousends of games
# The code does not implement the complete "AI" for the bods, as that was not in the time schedule. 

# --- ## ---
# Here is ChatGPTs summery of the code:
# This code uses a heuristic "potential" system that simulates distance to the winning edges for each player. 
# It repeatedly relaxes these potentials, then selects the move that minimizes the sum of the relevant potentials for the current player, 
# effectively choosing positions that bring it closer to a winning path.
# --- ## ---
...
class HexGame:
    ...
    def compute_potentials(self, level=2):
        ...
        # Initialize potentials for borders
        for ii in range(self.size):
            # Blue border: left/right
...
def simulate_one_game(size=11, swap_rule=False, level=2):
    ...
    while True:
        ...
        if game.is_full():
            # No empty cells, rare in Hex without a winner:
            return None, game.board

        move = game.select_best_move(level=level)
        if move is None:
            # No moves found
            return game.check_winner(), game.board
        game.make_move(*move)
...
def simulate_many_games(num_games=1000, size=3, swap_rule=True, level=3):
    print(f"Simulating size: {size}. Num games: {num_games}")
    
    results = {RED:0, BLUE:0, None:0}
    for i in range(num_games):
        w, final_board = simulate_one_game(size=size, swap_rule=swap_rule, level=level)
        results[w] += 1
        if w is not None:
            save_board_to_csv(final_board, w, filename="games.csv")
...
if __name__ == "__main__":
    simulate_many_games(num_games=4000, size=9, swap_rule=True, level=5)
